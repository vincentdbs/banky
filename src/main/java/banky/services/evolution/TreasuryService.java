...
public class TreasuryService {
     ...
     */
    private Map<String, TotalByAccountAndMonthResponse> mapToAnnualTotalResponse(List<AccountMonthlyEvolution> accountsMonthlyEvolution) {
            ...
            byAccountType.forEach((accountTypeStr, accountTypeEvolutions) -> {
                ...
                List<TotalByAccountResponse> totalByAccount = accountTypeEvolutions.stream()
                    .map(account -> {
                        ...
                        if (
                            account.totalAmount().compareTo(BigDecimal.ZERO) != 0
                                && account.gainLoss().compareTo(BigDecimal.ZERO) != 0
                                && account.totalAmount().subtract(account.gainLoss()).compareTo(BigDecimal.ZERO) != 0
                        ) {
